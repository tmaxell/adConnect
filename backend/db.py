"""Async DB setup + ChatStore repository (sessions, messages, runs, artifacts).

Defaults to a local SQLite file (zero setup). Set DATABASE_URL to point at
Postgres for shared/persistent deployments.
"""
...
_DEFAULT_SQLITE_PATH = Path(__file__).parent / "data" / "adconnect_agents.sqlite3"


def get_database_url() -> str:
    """SQLAlchemy URL for the store. Explicit DATABASE_URL wins; otherwise SQLite."""
...
async def init_db() -> None:
    """Create the schema if missing (idempotent). User data is never dropped."""
...
class ChatStore:
    ...
    async def get_session_with_messages(self, session_id: str) -> dict[str, Any] | None:
        async with session_scope() as db:
            ...
            payload["messages"] = [
                self._message_dict(
                    m,
                    trace=events_by_run.get(
                        (m.metadata_json or {}).get("run_id", "") if isinstance(m.metadata_json, dict) else "",
                        [],
                    ),
                )
                for m in messages
    ...
    async def ensure_session(
        ...
        async with session_scope() as db:
            ...
            if session is None:
                session = ChatSessionModel(id=sid, title=title or "Новый диалог")
                db.add(session)
                await db.flush()
